...
class DailyDischargeEvaluation(object):

    def __init__(self, modelOutputFolder, \
                       startDate=None, endDate=None, \
                       temporary_directory = None):
        object.__init__(self)
        
        logger.info('Evaluating the model results (daily discharge) stored in %s.', modelOutputFolder)
    ...
    def get_grdc_attributes(self, directoryGRDC):
        ...
        # This script needs daily observation files.
    ...
    def evaluateAllModelResults(self,\
        ...
        for id in self.list_of_grdc_ids: 
            ...
            if pcrglobwb_output["netcdf_file_name"].endswith(('.nc4', '.nc')): 
                # for the case that results are stored in a single netcdf file:
                self.evaluateModelResultsToGRDC(str(id), pcrglobwb_output, catchmentClassFileName, tmpDir)
            else:
                # for the case that results are stored in several netcdf files: 
                self.evaluate_model_results_in_several_yearly_netcdf_files_to_grdc_observation_data(str(id), pcrglobwb_output, catchmentClassFileName, tmpDir)
    ...
    def evaluate_model_results_in_several_yearly_netcdf_files_to_grdc_observation_data(self,id,pcrglobwb_output,catchmentClassFileName,tmpDir):
        
        year_list = range(self.startDate.year, self.endDate.year + 1, 1)  
        ...
        try:
            ...
            for year in year_list:
                ...
                ncFile = pcrglobwb_output['folder']+ "/" + pcrglobwb_output["netcdf_file_name"] + "_" + str(year) + ".nc"
                ...
                if catchmentClassFileName != None:
                    ...
                    ncFile = pcrglobwb_output['folder'] + "/M" +landmaskCode + "/" + pcrglobwb_output["netcdf_file_name"]
                ...
                cropTime = range(int(f.variables['time'][0]), int(f.variables['time'][-1]) + 1, 1) 
            ...
            self.attributeGRDC["num_of_daily_pairs"][str(id)]  = nPairs               
